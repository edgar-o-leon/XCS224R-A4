...
from .dream import get_env_class, run_episode


def main():
    arg_parser = argparse.ArgumentParser()
    arg_parser.add_argument(
            "-c", "--configs", action="append", default=["./meta_rl/configs/rl2.json"])
    arg_parser.add_argument(
            "-b", "--config_bindings", action="append", default=[],
            help="bindings to overwrite in the configs.")
    arg_parser.add_argument(
            "-x", "--base_dir", default="./meta_rl/experiments",
    ...
    config = Config.from_files_and_bindings(
