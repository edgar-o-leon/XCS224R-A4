...
from .grid import Action, GridEnv, Bus
...
class InstructionWrapper(meta_exploration.InstructionWrapper):
    ...
    def _instruction_observation_space(self):
        return gym.spaces.Box(
                np.array([0, 0]), np.array([self.unwrapped.width, self.unwrapped.height]),
                dtype=int)
    ...
    def _reward(self, instruction_state, action, original_reward):
        ...
        reward = -0.3
        if np.array_equal(self.unwrapped.agent_pos, instruction_state.instructions):
            reward = 1
        elif action == Action.end_episode:
            reward -= self.unwrapped.steps_remaining * 0.3  # penalize ending the episode

        done = any(np.array_equal(self.unwrapped.agent_pos, goal) for goal in self.GOALS)
        return reward, done
...
class CityGridEnv(GridEnv):
    ...
    def _place_objects(self):
        ...
        for (bus_stop, color), dest in zip(self._bus_sources, destinations):
            self.place(Bus(color, dest), bus_stop)
            self.place(Bus(color, bus_stop), dest)
