...
class EncoderDecoder(Embedder, relabel.RewardLabeler):
    ...
    def _compute_embeddings(self, trajectories):
        ...
        # pack_padded_sequence requires sequence_lengths on CPU (handled above)
        # No need to change default tensor type, just ensure sequence_lengths is on CPU
        ...
        initial_hidden_states = torch.zeros(
                ...
                device=transition_hidden_states.device)
        ...
        id_embeddings = self._id_embedder(
                torch.tensor(np.array([traj[0].state.env_id for traj in trajectories])))

        # don't mask the initial hidden states (batch_size, max_len + 1)
        mask = torch.cat(
                (torch.ones(decoder_embeddings.shape[0], 1, device=mask.device).bool(), mask), -1)
    ...
    def _compute_losses(
        ...
        cutoff = torch.ones(id_embeddings.shape[0], device=id_embeddings.device) * 10
        losses = {
